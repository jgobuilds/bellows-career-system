...
- Header name renders as **Firstname Lastname, M.S.** in Title Case. Do **NOT** use all-caps ("FIRSTNAME LASTNAME"): all-caps names parse worse on ATS import (some parsers mis-split or drop the name) and read as a template. Legal first name plus the preferred name in parentheses, so a background check matches the legal name while recruiters see the name they go by.
...
- **Reverse-chronological order, always.** Experience entries run most-recent first by end date, and stacked sub-roles under one employer descend the same way. "Present" is the most recent. Do NOT reorder by relevance to a posting — a scrambled timeline reads as an error and confuses ATS parsers, which cancels any emphasis benefit. To foreground a specific role for a target job, use the Professional Summary and the cover letter, not the work-history order. `resume_builder.validate()` flags any violation, and it dings the `resume_score` ATS-safe dimension.
- **Employer block: one datum per line, in this exact order, no fields combined** (parsers mis-map combined lines — a real ATS import proved it):
  ...
  3. **Location and dates together on their own line, pipe-separated, NOTHING but `City, ST | dates`** — e.g., `City, ST | May 2019 – June 2022`. Use `|` between location and dates, not `·` (the middot is nonstandard as a field separator). **Use the EMPLOYER's city, never your own** — an employer city is a verification signal that the company is real and findable, which matters most for the lesser-known names on the résumé. **Remote roles take a parenthetical: `Cleveland, OH (Remote) | June 2022 – September 2023` (standard adopted 2026-07-24).** A Connecticut candidate with an Ohio employer reads as remote to a human even before the parenthetical does its work. ⚠️ **Known Workday cost, accepted deliberately:** two Pfizer import tests (2026-07-16) showed `City, ST (Remote)` fails Workday's parser — it cannot match the parenthetical as a location, so the whole location-and-dates string leaks into the role description. The standard accepts that on the majority of applications that are not Workday; **on a Workday application, hand-correct the location field after import**, the same as the job title, which Workday normalizes regardless. A bare `Remote` with no city stays banned outright: it leaves the field blank AND tells the reader nothing.
...
- **Older roles go in the ONE Professional Experience section — no separate "Earlier Experience" block (changed 2026-07-22).** They are the oldest entries, so reverse-chron puts them last, and each still gets the normal per-role block (Company / Title / Location · Dates; a bullet is optional and usually absent for old roles). The reason for folding them in: three experience-like sections (Professional Experience, Advisory & Consulting, Earlier Experience) gave a résumé parser too much to segment, and on a Workday import it dropped the CURRENT role entirely. Fewer sections, cleaner parse. Never a run-in paragraph listing several old roles together, and never a "years supporting X" sentence inside experience — that belongs in the Professional Summary. Keep multi-word company names intact ("The Former Employer Life Insurance Company of America") so runs don't merge into "Companyof".
...
3. **Approval gate 2 — the "where I add value" body.** Draft the body paragraph(s) mapping their record to the role, get the yes, then assemble the full letter. Every claim traces to the profile (rule 0.1); metrics stay exact, never rounded up to sound better.
...
one open role. This is not hypothetical: one employer here has two live applications
(one folder per role, distinguished by seniority and specialism). A
